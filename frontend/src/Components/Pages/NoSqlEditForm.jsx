...
export const NoSqlEditForm = () => {
  ...
  const handleSubmit = (formData) => {
    ...
    fetch(`http://localhost:3000/nosql/update/${userInfo._id}`, {
      method: 'PUT',
      headers: {
  ...
  return (
    <div className='h-full border border-red-500 flex flex-col justify-center items-center'>
      <h1 className='mb-5 font-bold text-2xl'>NoSql Update entry for user: {userInfo.username}</h1>
      <GenericForm handleSubmit={handleSubmit} requiredAll={false}/>
    </div>
  );
}

export default NoSqlEditForm;